...
Canopen_socket::Canopen_socket(const char *ifname, bool verbose)
    : m_verbose(verbose)
{
...
Canopen_socket::print_frame(struct can_frame frame)
{
    ...
    if(frame.can_id > 0x580 && frame.can_id < 0x700)
    {
        ...
        else
        {
            ...
            printf(" ) \t "
                   "%s\tIndex:0x%04X\tSubindex:0x%"
                   "02X\tData:%d (0x%04X)\n",
                   ((frame.data[0] & 0xf0) == 0x40) ? "READ" : "WRITE",
                   *(uint16_t *)(frame.data + 1), *(uint8_t *)(frame.data + 3),
                   (sd == 1) ? *(uint8_t *)(frame.data + 4)
                             : (sd == 2) ? *(uint16_t *)(frame.data + 4)
